...
void renderer_d3d9::update_presentation_parameters()
{
	...
	m_presentation.Windowed = !win->fullscreen() || GetMenu(std::static_pointer_cast<win_window_info>(win)->platform_window()); // MESSUI
...
int renderer_d3d9::config_adapter_mode()
{
	...
	if (!win->fullscreen() || !video_config.switchres || GetMenu(std::static_pointer_cast<win_window_info>(win)->platform_window())) // MESSUI
	{
		RECT client;
